...
const SearchModal: React.FC<{
    ...
    const categorySearchResult = useMemo(() => {
        ...
        const result = filteredTransactions.filter(tx => {
            ...
            <div className="modal-content large">
                ...
                <div className="search-results">
                    ...
                    {categorySearchResult && (
                        <><h3>{categoryQuery === 'ALL' ? (categoryType === 'income' ? '입금 전체' : '출금 전체') : categoryQuery} 내역 (총: {categorySearchResult.total.toLocaleString()}원)</h3>
                        <ul>{categorySearchResult.transactions.map(tx => (
                            <li key={tx.id}>
                                {tx.date} | {categoryQuery === 'ALL' ? `[${tx.category}] ` : ''}{tx.type === 'income' ? getMemberName(tx.memberId) : (tx.memo || '메모 없음')}: {tx.amount.toLocaleString()}원
                            </li>
                        ))}</ul></>
                    )}
                    {amountSearchResult && (
